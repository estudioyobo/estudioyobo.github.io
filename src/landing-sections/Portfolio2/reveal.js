...
export class GridItem {
  constructor(el, options) {
    ...
    this.revealer = new Revealer(this.DOM.el, {
      color: this.CONFIG.filledColor || "#333"
    });
    this.initEvents();
  }
  initEvents() {
  ...
  show(animation = true) {
    return animation
      ? this.revealer.show({
          direction: this.DOM.el.dataset.direction || "ltr",
...
export class Grid {
  constructor(el, options) {
    ...
    this.textElems = [this.DOM.name, this.DOM.title, this.DOM.text].filter(
      el => el !== null
    );
    this.layout();
  }
...
export class Slideshow {
  ...
  initEvents() {
    Array.from(document.querySelectorAll(".grid__item--nav-next")).forEach(
      ctrl => ctrl.addEventListener("click", ev => this.navigate(ev, "next"))
    );
    Array.from(document.querySelectorAll(".grid__item--nav-prev")).forEach(
      ctrl => ctrl.addEventListener("click", ev => this.navigate(ev, "prev"))
    );
  ...
  animate(animationOpts, action) {
    setTimeout(() => {
      ...
      target.classList.add(
        dirClass,
        orientation === "h" ? `revealer--${action}X` : `revealer--${action}Y`
      );
